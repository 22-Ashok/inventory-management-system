...
export const editUserByStaffSchema = z.object({
    body: z.object({
        email: z.string().email({ message: "Invalid email address" }).optional(),
        f_name: z.string().min(1, { message: "First name is required" }).optional(),
        l_name: z.string().min(1, { message: "Last name is required" }).optional(),
...
export type EditUserByStaffBodyType = z.infer<typeof editUserByStaffSchema>['body'];
export type EditUserByStaffParamType = z.infer<typeof editUserByStaffSchema>['params'];